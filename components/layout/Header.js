...
const Header = () => {
  ...
  return (
    <header className="flex w-full h-full relative ">
      <div
        className={`fixed rounded-l-xl w-56 p-5 h-full z-20 top-0 right-0 bg-white text-black text-xl font-bold transition-transform duration-300 
      ...
      >
        ...
        <div className="flex flex-col mt-4 space-y-2">
          <button className="flex p-2 border-b  w-full">Home</button>
          <div className="flex flex-col p-2 border-b w-full">
            ...
            <ul
              className={`flex-col text-base font-normal space-y-2 transition-all duration-700 ease-in  ${
                !openTop ? "h-0  overflow-hidden" : "h-auto"
              }`}
            >
              <li>
              ...
              <li>
                <Link href="/best-of-the-year">Best Of The Year</Link>
              </li>
              <li>
          ...
          <ul className="flex p-2 border-b w-full">
            <li className="flex items-center justify-between flex-grow">
              <span>Platforms</span>
      ...
      <div
        className={`fixed bg-[rgba(0,0,0,.8)] top-0 right-0 left-0 bottom-0 z-10 transition-all ${
          open ? "block" : "hidden"
        }`}
      ...
      <div className="flex px-4 py-3 md:px-9 md:py-5  flex-grow justify-between items-center">
        ...
        <div className='className="flex flex-grow h-10 md:h-16  mr-5 relative'>
          ...
          {qry && (
            <div className="flex flex-col space-y-5 p-5 absolute z-50 -bottom-24.5 w-full min-h-[100px] max-h-[27rem] bg-black rounded-2xl overflow-y-auto">
              {loading
                ? "Loading ..."
                : games.map((game) => (
                    <SearchGameCard
        ...
        <div className="flex h-10 md:h-16 justify-center items-center mr-5">
          {session && (
            <div className="flex justify-center items-center w-8 h-8 md:w-9 md:h-9 rounded-full bg-gradient-to-r from-[#b1305c] to-[#fd867d]">
